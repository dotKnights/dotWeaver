...
function toolResultText(content: unknown): string {
	...
	if (Array.isArray(content)) {
		return content.map((c) => (typeof c === 'string' ? c : typeof asObj(c).text === 'string' ? (asObj(c).text as string) : JSON.stringify(c))).join('\n');
	}
...
export function normalizeEvent(payload: unknown): DisplayEvent[] {
	try {
		...
		if (type === 'user') {
			...
			for (const raw of items) {
				const c = asObj(raw);
				if (c.type === 'tool_result') out.push({ kind: 'tool_result', text: toolResultText(c.content), isError: c.is_error === true });
				else if (c.type === 'text') out.push({ kind: 'assistant_text', markdown: String(c.text ?? '') });
			}
			return out.length ? out : [{ kind: 'hidden' }];
		}
