<script lang="ts">
	...
	const capabilities = $derived(getProjectCapabilities(page.params.id!));
	const myTeams = listMyTeams();
	const hasInternalTeams = $derived(myTeams.current?.hasInternalTeams ?? false);
	const canViewConfig = $derived(capabilities.current?.['project.config.view'] ?? false);
	const canManageConfig = $derived(capabilities.current?.['project.config.manage'] ?? false);
	const canManageAccess = $derived(
		hasInternalTeams && (capabilities.current?.['project.manage_access'] ?? false)
	);
	const canViewRuns = $derived(capabilities.current?.['run.view'] ?? false);
	const canCreateRuns = $derived(capabilities.current?.['run.create'] ?? false);
	const branches = $derived(canCreateRuns ? listProjectBranches(page.params.id!) : null);
	const agentConfig = $derived(canViewConfig ? getProjectAgentConfig(page.params.id!) : null);
	const environment = $derived(canViewConfig ? getProjectEnvironment(page.params.id!) : null);
	const environmentProfileId = $derived(environment?.current?.id ?? '');
	const environmentPrepareEvents = $derived(
		canViewConfig && environmentProfileId
			? getProjectEnvironmentPrepareEvents({
	...
	const liveEnvironment = createProjectEnvironmentLiveState({
		projectId: () => page.params.id!,
		profileId: () => environmentProfileId,
		environment: () => environment?.current ?? null,
	...
	const runs = $derived(canViewRuns ? listRuns(page.params.id!) : null);
	...
	const canPassSetupGate = $derived(!canViewConfig || canOpenProject);
	const canStartRun = $derived(
		canCreateRuns && canPassSetupGate && !starting && !!prompt.trim() && !!selectedBaseBranch
	);

	async function handleStart() {
		if (!prompt.trim() || !selectedBaseBranch || !canCreateRuns || !canPassSetupGate) return;
		startError = null;
		starting = true;
		try {
			await startRun({
...
<div class="mx-auto max-w-5xl space-y-6 p-6">
	...
	{:else if project.current}
		...
		{#if capabilities.error}
			<p class="text-sm text-red-500">{capabilities.error.message}</p>
		{:else if capabilities.current}
			{#if canViewConfig}
				...
				{:else if environment?.current !== undefined}
					{#key `${page.params.id}:${environment.current?.id ?? 'none'}`}
						{#if canManageConfig && setupState.primaryAction !== 'open_project'}
							<div
								class="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950"
							>
			...
			{#if canManageAccess}
				<ClientAccessPanel projectId={page.params.id!} {canManageAccess} />
			{/if}
						...
						<div class="w-full space-y-1 sm:w-52">
							...
							{#if branches?.error}
								<p class="text-xs text-destructive">
									Could not load branches. Default branch only.
								</p>
							{/if}
						</div>
			...
			{/if}
		{:else}
			<p class="text-sm text-muted-foreground">Loading project permissions…</p>
		{/if}
	{:else}
