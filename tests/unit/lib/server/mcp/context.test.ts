...
const findMany = vi.mocked(prisma.member.findMany) as unknown as Mock<() => Promise<Membership[]>>;

describe('resolveOrgContext', () => {
	beforeEach(() => vi.clearAllMocks());

	it('une seule org, pas de team -> defaut sur cette org', async () => {
		findMany.mockResolvedValue([membership('acme', 'org1')]);
		expect(await resolveOrgContext('u1')).toBe('org1');
	});

	it('team fourni et membre -> cette org', async () => {
		findMany.mockResolvedValue([membership('acme', 'org1'), membership('globex', 'org2')]);
